...
var app4 = new Vue({
    ...
    methods: {
        ...
        /**
         * Metodo para buscar los hoteles segun una ciudad
         */
        obtener_hoteles() {
            vue_global.ajax_peticion('datos/listar_hoteles', { ciudad: this.municipioSelect }, [(respuesta) => {
                if (respuesta.estado == 1) {
                    console.log(respuesta.datos);
                    this.hoteles = respuesta.datos;
                } else {
         ...
         */
        obtener_habitacionesdisponibles(id) {

            vue_global.ajax_peticion('datos/habitacionesdisponibles', { idhotel: id }, [(respuesta) => {
                if (respuesta.estado == 1) {
                    console.log(respuesta.datos);
                    this.habitacionesdisponibles = respuesta.datos;
                } else {
                    alert('Hubo un problema con la carga de la informaci\u00f3n');
        ...
        /**
         * metodo para registra la reserva
         */
        registrar_reserva(id) {

            $('#myModal').modal('show');
            this.registro.idhoteles = id;

        },


        /**
        * Este metodo registra la reservaca enviado la informacion bindeada en this.registro 
        ...
        */
        asignar_reserva() {
            ...
            swal({
                title: 'Información',
                text: "Desea registrar la reserva ?",
                type: 'warning',
                ...
                vue_global.ajax_peticion('reservas/registrar_reserva', this.registro, [(respuesta) => {
        ...
        /**
         * metodo para cambiar el dato condicion        
         */
        cambiarCondicion() {
            this.condicion = 1;
        },
        ...
        mostrar_imagenes(id) {
            ...
            location.href = "/mostrarImagenes/" + id;
            // $direccion='/mostrarImagenes'

            // vue_global.ajax_peticion('/mostrarImagenes', id, [(respuesta) => {

            //         console.log(respuesta.datos);
            //         this.imagenes = respuesta.datos;

            // }]);
            // $('#mostrarIma').modal('show');
        ...
        filteredList() {
            ...
            if (this.opcionbusqueda == "nombre") {
                return this.hoteles.filter(hotel => {
                    return hotel.nombre.toLowerCase().includes(this.search.toLowerCase())
                })
            }
            if (this.opcionbusqueda == "camadoble1persona") {
                return this.hoteles.filter(hotel => {
                    return hotel.camadoble1persona >= parseInt(this.search);
                })
            }
            if (this.opcionbusqueda == "camadoble2persona") {
                return this.hoteles.filter(hotel => {
                    this.search == 1;
                    return hotel.camadoble2persona >= parseInt(this.search);
                })
            }
            if (this.opcionbusqueda == "camarote") {
                return this.hoteles.filter(hotel => {
                    return hotel.camarote >= parseInt(this.search);
                })
            }
            if (this.opcionbusqueda == "parqueadero") {
                return this.hoteles.filter(hotel => {
                    return hotel.parqueadero.toLowerCase().includes(this.search.toLowerCase())
                })
            }
            if (this.opcionbusqueda == "wifi") {
                return this.hoteles.filter(hotel => {
                    return hotel.wifi.toLowerCase().includes(this.search.toLowerCase())
                })
            }
        }
    }
